...
export default async function Home({params}:any) {
  ...
  const query = await getData(`{
    'project':*[_type=="project" && slug.current == '${slug}']{"logoUrl":logo.asset->url,title,desc,youtube, slug,"work":content[]{desc,"imageUrl":image.asset->url, video{asset->{playbackId}}}, social, status}
    }`)

  const {project} = query.data
  console.log(project, slug,params.params)
...
<React.Fragment>
    ...
    {project[0]?(<Reveal styleSet="w-full grid grid-cols-12 py-[160px]">
...
export async function generateMetadata({params}:any) {
 ...
 const {data,project} = query.data  
  return {
    title: `${project?project.title:''} | 400M: Productions`,
    openGraph: {
